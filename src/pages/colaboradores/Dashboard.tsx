        ...
        <div className="text-center mb-12">
          <div className="inline-flex items-center bg-white px-6 py-4 rounded-2xl shadow-xl ring-1 ring-gray-900/5">
            ...
            <div>
              ...
              <p className="text-lg text-gray-600 mt-1">Matrícula: <span className="font-semibold text-blue-600">{user?.matricula}</span></p>
              <button
                onClick={logout}
          ...
          <div className="bg-white rounded-2xl shadow-xl p-8">
            ...
            <div className="space-y-3">
              {escala.entradas_saidas.length > 0 ? (
                escala.entradas_saidas.map((registro, index) => (
                  <div key={index} className="p-4 bg-gray-50 rounded-xl border-l-4 border-blue-500">
                    <div className="flex justify-between items-center">
                      <span className="font-semibold text-gray-900">{new Date(registro.data).toLocaleDateString('pt-BR')}</span>
                ...
                <p className="text-gray-500 text-center py-8">Nenhum registro de ponto encontrado</p>
              )}
            </div>
          </div>
          ...
          <div className="space-y-6">
            ...
            <div className="bg-gradient-to-r from-yellow-50 to-orange-50 rounded-2xl p-8 shadow-xl border-2 border-yellow-200">
              <div className="flex items-center">
                ...
                <div>
                  <p className="text-sm font-medium text-yellow-800">Horas Extras</p>
                  <p className="text-3xl font-bold text-yellow-900">{escala.horas_extras}h</p>
                </div>
              </div>
            </div>
            ...
            <div className="bg-gradient-to-r from-green-50 to-emerald-50 rounded-2xl p-8 shadow-xl border-2 border-green-200">
              <div className="flex items-center">
                ...
                <div>
                  <p className="text-sm font-medium text-green-800">Próximas Folgas</p>
                  <p className="text-lg font-bold text-green-900">
                    {escala.folgas.length > 0 
                      ? escala.folgas.map(f => new Date(f).toLocaleDateString('pt-BR')).join(', ')
                      : 'Nenhuma folga'
                    }
                  </p>
                </div>
